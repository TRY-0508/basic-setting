...
- 当前用户自建的 skill：`karpathy-guidelines`、`project-workflow`、`consistency-management`、`knowledge-accumulation`、`external-research`、`paper-survey`、`chinese-academic-writing`
...
所有 AI 生成的文档（设计文档、分析报告、综述、提案、经验总结等）必须遵循 `project-workflow` skill 中的编写规范。核心要求：

- **避免术语堆砌**：一句话需要读者解析三个以上堆叠的专业名词时，拆成短句或用示例说明
- **使用平实语言**：写得像在向一位没读过代码但有能力的工程师解释。不用学术化表达
- **不用虚饰限定词**：禁止"全项目唯一权威定义""企业级生产就绪""最佳实践"等膨胀重要性的词组
- **具体胜过抽象**："系统是可扩展的" → 说出如何扩展；"有清晰的边界" → 说出边界在哪里
